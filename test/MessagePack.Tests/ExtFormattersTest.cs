...
{
    public class ExtFormattersTest
    {
        [Fact]
        ...
        {
            var dts = DateTimeOffset.Now;

            var idx = 0;
            var writer = new MessagePackWriter(16);
            ExtDateTimeOffsetFormatter.Instance.Serialize(ref writer, ref idx, dts, null);
            var buffer = writer.ToArray(idx);

            var reader = new MessagePackReader(buffer);
            ExtDateTimeOffsetFormatter.Instance.Deserialize(ref reader, null).Is(dts);
        }
