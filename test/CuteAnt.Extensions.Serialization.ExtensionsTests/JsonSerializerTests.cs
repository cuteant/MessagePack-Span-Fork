...
namespace CuteAnt.Extensions.Serialization.Tests
{
  public class JsonSerializerTests
  {
    ...
    public void JsonObjectTypeDeserializer_Test()
    {
      ...
      Assert.Equal(combGuid, jsonStr.Deserialize<CombGuid>());
      Assert.Equal(combGuid, combGuid.Deserialize<CombGuid>());
      Assert.Equal((Guid)combGuid, combGuid.Deserialize<Guid>());
      Assert.Equal(combGuid.ToString(), combGuid.Deserialize<string>());
      Assert.Equal(combGuid.ToByteArray(), combGuid.Deserialize<byte[]>());
      Assert.Equal(combGuid.DateTime, combGuid.Deserialize<DateTime>());
      Assert.Equal(DateTime.UtcNow.Date, combGuid.Deserialize<DateTime>().Date);
      Assert.Equal(new DateTimeOffset(combGuid.DateTime), combGuid.Deserialize<DateTimeOffset>());
    }
