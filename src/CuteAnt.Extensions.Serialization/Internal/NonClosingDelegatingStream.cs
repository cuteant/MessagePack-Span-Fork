...
{
  /// <summary>Stream that doesn't close the inner stream when closed. This is to work around a limitation
  ...
  /// <see cref="CuteAnt.Extensions.Serialization.IMessageFormatter"/>
  /// should not close the input stream when reading or writing so hence this workaround.</summary>
  internal class NonClosingDelegatingStream : DelegatingStream
  {
    internal NonClosingDelegatingStream(Stream innerStream)
      : base(innerStream)
    {
    }
