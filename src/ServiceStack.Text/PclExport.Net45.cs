...
namespace ServiceStack
{
    ...
    public static class PclExportExt
    {
        ...
        {
            var attr = type.GetAllAttributes().FirstOrDefault(x => x.GetType().Name == DataContract);
            if (attr != null)
            {
        ...
        {
            var attr = pi.GetAllAttributes().FirstOrDefault(x => x.GetType().Name == ReflectionExtensions.DataMember);
            if (attr != null)
            {
                var attrType = attr.GetType();
