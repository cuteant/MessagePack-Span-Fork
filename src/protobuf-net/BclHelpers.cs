...
//#endif
    ...
    /// <summary>
    /// Parses a CombGuid from a protobuf stream
    /// </summary>
    public static CuteAnt.CombGuid ReadCombGuid(ProtoReader source)
    {
      ulong low = 0, high = 0;
      int fieldNumber;
      SubItemToken token = ProtoReader.StartSubItem(source);
      ...
      ProtoReader.EndSubItem(token, source);
      if (low == 0 && high == 0) return CuteAnt.CombGuid.Empty;
      uint a = (uint)(low >> 32), b = (uint)low, c = (uint)(high >> 32), d = (uint)high;
      return new CuteAnt.CombGuid((int)b, (short)a, (short)(a >> 16),
          (byte)d, (byte)(d >> 8), (byte)(d >> 16), (byte)(d >> 24),
          (byte)c, (byte)(c >> 8), (byte)(c >> 16), (byte)(c >> 24));
