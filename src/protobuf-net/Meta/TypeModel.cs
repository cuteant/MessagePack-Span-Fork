      ...
      throw new NotSupportedException(); // this should come from RuntimeTypeModel!
        ...
        internal bool TrySerializeAuxiliaryType(ProtoWriter writer, Type type, DataFormat format, int tag, object value, bool isInsideList, object parentList)
        {
            ...
            switch (typecode)
            {
                ...
                #region ## 苦竹 添加 ##
                case ProtoTypeCode.CombGuid: BclHelpers.WriteCombGuid((CuteAnt.CombGuid)value, writer); return true;
                #endregion
        ...
        public object DeserializeWithLengthPrefix(Stream source, object value, Type type, PrefixStyle style, int expectedField, Serializer.TypeResolver resolver, out long bytesRead)
            => DeserializeWithLengthPrefix(source, value, type, style, expectedField, resolver, out bytesRead, out bool haveObject, null);
      ...
      if (!listType.IsClass || listType.IsAbstract ||
        ...
        if (!handled)
        {
          ...
          handled = true;
            ...
            return CuteAnt.Reflection.ActivatorUtils.FastCreateInstance(concreteListType);
        }
        ...
        internal bool TryDeserializeAuxiliaryType(ProtoReader reader, DataFormat format, int tag, Type type, ref object value, bool skipOtherFields, bool asListItem, bool autoCreate, bool insideList, object parentListOrType)
        {
            ...
            while (true)
            {
                ...
                switch (typecode)
                {
                    ...
                    #region ## 苦竹 添加 ##
                    case ProtoTypeCode.CombGuid: value = BclHelpers.ReadCombGuid(reader); continue;
                    #endregion
            ...
            if (!found && !asListItem && autoCreate)
            {
                if (type != typeof(string))
                {
                    ...
                    value = CuteAnt.Reflection.ActivatorUtils.FastCreateInstance(type);
                }
            }
            return found;
        }
